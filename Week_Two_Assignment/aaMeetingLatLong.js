...
var aaMeetingArray= [];
var fullLLArray = JSON.parse(fs.readFileSync('meetingsData.json', 'utf8'));
//function
var building;
    var title;
    var stAdd;
...
$('table tbody tr td').each(function(i, elem) { 
    ...
    if ($(elem).attr('style') == "border-bottom:1px solid #e3e3e3; width:260px") {
        aaMeetingArray.forEach( function (latLong) {
    ...
    if ($(elem).attr('style') == "border-bottom:1px solid #e3e3e3;width:350px;") {
        ...
        for (var j=2; j < (count-3); j=j+4) {       //start creating meeting IDs
            ...
            for (var k=10; k < count2; k++) {
                ...
                aaMeetingArray.push(new enterData(meetingId, stAdd, title, building, rmAdd, wheelchair, notes, llCoor, googAdd, weekday, pStartTime, pEndTime, startHour, startMin, endHour, endMin, meetingType, special));
            }
            //aaMeetingArray.push(new enterData(meetingId, stAdd, title, building, rmAdd, wheelchair, notes, weekday, pStartTime, pEndTime, startHour, startMin, endHour, endMin, meetingType, special));
...
fs.writeFileSync('aaSchedule.json', JSON.stringify(aaMeetingArray)); //changed to json.
...
function enterData(_meetingId, _stAdd, _title, _building, _rmAdd, _wheelchair, _notes, _llCoor, _googAdd, _weekday, _pStartTime, _pEndTime, _startHour, _startMin, _endHour, _endMin, _meetingType, _special) {
    var private = {};
